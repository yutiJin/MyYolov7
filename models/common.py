...
class SPPFCSPC(nn.Module):
    ...
    def forward(self, x):
        ...
        y11 = self.m(x1)
        y12 = self.m(y11)
        y1 = self.cv6(self.cv5(torch.cat([x1, y11, y12, self.m(y12)], 1)))  # 左侧经过SPPF处理后的分支
        y2 = self.cv2(x)   # 最右侧的分支
        return self.cv7(torch.cat((y1, y2), dim=1))
# -------------- 1、 第一次改进：自己添加的SPPFCSPC ----------------------- #

# -------------- 1'、 第一次改进：自己添加的SPPFCSPCDW ----------------------- #
class SPPFCSPCDW(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=5):
        super(SPPFCSPCDW, self).__init__()
        c_ = int(2 * c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
...
# -------------- 2、 第二次改进：自己添加的SPPFBCSPC（名字后面得重新取） ----------------------- #
class SPPFBCSPC(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=5):
        super(SPPFBCSPC, self).__init__()
        c_ = int(2 * c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv12 = Conv(c1, c_, 1, 1)  # 中间增加的卷积
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(c_, c_, 3, 1)
        self.cv4 = Conv(c_, c_, 1, 1)
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)
        self.cv5 = Conv(4 * c_, c_, 1, 1)
        self.cv52 = Conv(4 * c_, c_, 1, 1)
        self.cv6 = Conv(c_, c_, 3, 1)
        self.cv7 = Conv(3 * c_, c2, 1, 1)

    def forward(self, x):
        x1 = self.cv4(self.cv3(self.cv1(x)))  # 最左侧三次卷积
        y11 = self.m(x1)
        y12 = self.m(y11)
        y1 = self.cv6(self.cv5(torch.cat([x1, y11, y12, self.m(y12)], 1)))  # 左侧经过SPPF处理后的分支
        x2 = self.cv12(x)  # 中间只进行一次卷积
        y21 = self.m(x2)
        y22 = self.m(y21)
        y2 = self.cv52(torch.cat([x2, y21, y22, self.m(y22)], 1))  # 中间经过SPPBootlenck处理后的分支
        y3 = self.cv2(x)   # 最右侧的分支
        return self.cv7(torch.cat((y1, y2, y3), dim=1))
# -------------- 2、 第二次改进：自己添加的SPPFBCSPC（名字后面得重新取）  ----------------------- #

# -------------- 3、 第二次改进2：自己添加的SPPFBRCSPC（名字后面得重新取，使用了DW）  ----------------------- #
class SPPFBRCSPC(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=5):
        super(SPPFBRCSPC, self).__init__()
        c_ = int(2 * c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = Conv(c_, c_, 3, 1)
    ...
    def forward(self, x):
        ...
        y12 = self.m(y11)
        y1 = self.cv6(self.cv5(torch.cat([x1, y11, y12, self.m(y12)], 1)))  # 左侧经过SPPF处理后的分支
        x2 = self.cv2(x)  # 中间只进行一次卷积
        y21 = self.m(x2)
        y22 = self.m(y21)
        y2 = self.cv52(torch.cat([x2, y21, y22, self.m(y22)], 1))  # 中间经过SPPBootlenck处理后的分支
        y3 = self.cv8(x)  # 残差网络的最右侧分支(用了DW)

        return self.cv7(torch.cat((y1, y2, y3), dim=1))
# -------------- 3、 第二次改进2：自己添加的SPPFBRCSPC（名字后面得重新取，使用了DW）  ----------------------- #

# -------------- 4、 第三次改进：自己添加的SPPFBRCSPCDW（名字后面得重新取） DWConv ----------------------- #
class SPPFBRCSPCDW(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=5):
        super(SPPFBRCSPCDW, self).__init__()
        c_ = int(2 * c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = DWConv(c1, c_, 1, 1)
        self.cv3 = DWConv(c_, c_, 3, 1)
        self.cv4 = Conv(c_, c_, 1, 1)
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)
        self.cv5 = Conv(4 * c_, c_, 1, 1)
        self.cv52 = DWConv(4 * c_, c_, 1, 1)
        self.cv6 = DWConv(c_, c_, 3, 1)
        self.cv7 = Conv(3 * c_, c2, 1, 1)
        self.cv8 = DWConv(c1, c_, 1, 1)
    ...
    def forward(self, x):
        ...
        y22 = self.m(y21)
        y2 = self.cv52(torch.cat([x2, y21, y22, self.m(y22)], 1))  # 中间经过SPPBootlenck处理后的分支
        y3 = self.cv8(x)  # 残差网络的最右侧分支

        return self.cv7(torch.cat((y1, y2, y3), dim=1))
# -------------- 4、 第三次改进：自己添加的SPPFBRCSPCDW（名字后面得重新取）  ----------------------- #

# -------------- 5、 第三次改进2：自己添加的SPPFBRCSPCDWL（名字后面得重新取） 只有最左边两侧 ----------------------- #
class SPPFBRCSPCDWL(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=5):
        super(SPPFBRCSPCDWL, self).__init__()
        c_ = int(2 * c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = DWConv(c1, c_, 1, 1)
        self.cv3 = DWConv(c_, c_, 3, 1)
    ...
    def forward(self, x):
        ...
        return self.cv7(torch.cat((y1, y2), dim=1))
# -------------- 5、 第三次改进2：自己添加的SPPFBRCSPCDW（名字后面得重新取） 只有最左边两侧 ----------------------- #

# -------------- 6、 第三次改进3：自己添加的SPPFBRCSPCDWLR（名字后面得重新取） 最左侧+最右侧 ----------------------- #
class SPPFBRCSPCDWLR(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5, k=5):
        super(SPPFBRCSPCDWLR, self).__init__()
        c_ = int(2 * c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = DWConv(c1, c_, 1, 1)
        self.cv3 = DWConv(c_, c_, 3, 1)
    ...
    def forward(self, x):
        ...
        y12 = self.m(y11)
        y1 = self.cv6(self.cv5(torch.cat([x1, y11, y12, self.m(y12)], 1)))  # 左侧经过SPPF处理后的分支
        y2 = self.cv2(x)  # 残差网络的最右侧分支

        return self.cv7(torch.cat((y1, y2), dim=1))
# -------------- 4、 第三次改进3：自己添加的SPPFBRCSPCDW（名字后面得重新取）最左侧+最右侧  ----------------------- #
